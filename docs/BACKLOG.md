...
| **Pivote a app Android + Firebase (v0.6.0)** — Inc. 4: Migración del progreso `localStorage` existente a la nube | `feat/migracion-progreso-local` | 🟡 **En curso — specs entregadas.** Specs en `docs/specs/07` (Épica E, US-E1…US-E8). Cubre: aviso explícito de migración al crear el primer perfil (Modelo A y Modelo B); **verify-before-delete** (no borrar el progreso local hasta confirmar la subida en Firestore leyéndola de vuelta); migración de **todos los cursos** con avance, no solo el activo (`courses` de `storage.ts` → documentos de curso por perfil); **una sola vez por dispositivo** (marcador en `localStorage`, sin fusión entre hermanos); **reanudación/reintento** si el alta se cancela o la verificación falla (idempotente, no bloquea el juego, dato local conservado); **cambio de cuenta de tutor** en el mismo dispositivo (control en `SettingsScreen` protegido por reto de adulto — hoy solo hay `signOut` en el selector con varios hijos y en la verificación de email). Sin bump: dentro de 0.6.0 del pivote. **Pendiente:** Arquitecto/UX-UI/Maquetador/Frontend + gates. Input para Abogado: validar que el consentimiento vigente cubre el traslado a la nube de progreso preexistente en el dispositivo |
