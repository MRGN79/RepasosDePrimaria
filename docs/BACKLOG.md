...
| **Fix: Google Sign-In nativo (ADR-006)** — `signInWithPopup` no funciona en el WebView de Capacitor (confirmado por el usuario en un dispositivo real: se abre un navegador externo y la app se queda colgada). Se sustituye por `@capacitor-firebase/authentication` en modo `skipNativeAuth` (picker nativo, la sesión la sigue gestionando el SDK Web de Firebase) | `fix/google-signin-nativo` (Paso 1, mergeada) → `fix/google-signin-wire` (Paso 2, mergeada) | 🔄 Código completo, **falta el Paso 3 de verificación en dispositivo real** antes de dar el fix por cerrado. **Paso 0** ✅: SHA-1 del keystore de debug registrado en Firebase Console. **Paso 1** ✅ **Mergeado en `main`** (PR #41, `0eb6fdd`): dependencia instalada, `cap sync` registra los módulos nativos, CI inyecta `google-services.json`. Efecto colateral corregido de paso: `@capacitor/preferences` (PIN de pestillo, Inc. 3) nunca se había registrado en el proyecto Android nativo. **Paso 2** ✅ **Mergeado en `main`** (PR #42, `92da26d`): `signInWithGoogle()` cableado de verdad — plataforma nativa usa el picker (`skipNativeAuth: true`) y entrega el idToken a `signInWithCredential` del SDK Web; detección de cancelación (`errors.cancelled`). **Gates:** QA ✅ · Seguridad ✅ · Abogado ✅. **Hallazgo crítico de Seguridad, corregido en el mismo PR**: `rgcfaIncludeGoogle` no estaba activado en `android/variables.gradle` — sin él, el picker nativo habría fallado en el dispositivo con `NoClassDefFoundError` pese a que el código pasaba todos los gates. Corregido contra la guía oficial del plugin. **Pendiente — Paso 3**: el usuario instala el APK de este merge en su tablet y verifica rol tutor, rol niño, cancelación del picker, sin red, y cuentas Family Link. El `CHANGELOG.md` de cara al usuario se actualiza solo cuando esto se confirme — no antes |
